...
def autogel_getmodel(in_features, out_features,gpu=0,seed=10):
...
def derive_arch(model):
    ...
    return model
def retrain(model, train_loader, val_loader, test_loader, train_mask, val_mask, test_mask, args):
    ...
    print('(Retrain Stage) best val acc: %.4f (epoch: %d)' % recorder.get_best_acc_val())
    print('(Retrain Stage) best test acc: %.4f (epoch: %d)' % recorder.get_best_acc())
    ...
    ans = {'valid_perf': val_results , 'test_perf': results}
    return model, ans
...
def main_autogel(searched_arch_op, dataname, seed ,gpu = 0 ):
    ...
    if args.dataset != 'PPI':
        dataset, in_features, out_features = get_data(args=args)
        train_mask, val_mask, test_mask = reset_mask(dataset, args=args, stratify=None)
...
def main(dataname,seed,gpu=0):
    ...
    model, results = search(model, train_loader, val_loader, train_mask, val_mask, args)
    model, results = retrain(model, train_loader, val_loader, test_loader, train_mask, val_mask, test_mask, args)
