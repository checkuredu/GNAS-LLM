...
def reset_mask(dataset, args, stratify=None):
    if args.task == 'graph':
        train_mask, val_mask, test_mask = None, None, None
        ...
        val_indices, test_indices = train_test_split(test_indices, test_size=args.test_ratio / (args.val_ratio + args.test_ratio),
                                                     stratify=stratify)
        print('Train size :{}, val size: {}, test size: {}, val ratio: {}, test ratio: {}'.format(len(train_indices),
                                                                                                        len(val_indices),
                                                                                                        len(test_indices),
                                                                                                        args.val_ratio,
                                                                                                        args.test_ratio))
...
def get_data(args):
    if args.task == 'node':
        dataset = Planetoid(root='/data4/zhengxin/code/AutoGEL/nc_gc/nc_gc/data/' + args.task + '/' + args.dataset, name=args.dataset)
    elif args.task == 'graph':
        dataset = TUDataset(root='/data4/zhengxin/code/AutoGEL/nc_gc/nc_gc/data/' + args.task + '/' + args.dataset, name=args.dataset)
    else:
        raise NotImplementedError
