...
def search(model, train_loader, val_loader, train_mask, val_mask, args):
    start = time.time()
    ...
    print('Search time for {} : {} seconds' .format(args.dataset, search_time))
...
def retrain(model, train_loader, val_loader, test_loader, train_mask, val_mask, test_mask, args, logger):
    ...
    for step in range(args.retrain_epoch):
        ...
        #              metric, recorder.get_latest_metrics()[0],
        #              metric, recorder.get_latest_metrics()[1]))
    logger.info('(Retrain Stage) best valid acc: %.4f (epoch: %d)' % recorder.get_best_metric_val())
    ...
    results = {'test_acc':recorder.get_best_acc(),'valid_acc':recorder.get_best_acc_val()}
    return model, results
...
class RetrainRecorder:
    ...
    def get_best_auc(self):
        ...
        max_step = int(np.argmax(np.array(self.test_aucs)))
        return self.test_aucs[max_step], max_step
    def get_best_metric_val(self):
        dic = {'acc': self.get_best_acc_val(), 'auc': self.get_best_auc_val()}
        return dic[self.metric]

    def get_best_acc_val(self):
        max_step = int(np.argmax(np.array(self.val_accs)))
        return self.val_accs[max_step], max_step

    def get_best_auc_val(self):
        max_step = int(np.argmax(np.array(self.val_auc)))
        return self.val_auc[max_step], max_step
    def get_latest_metrics(self):
        if len(self.train_accs) < 0:
            raise Exception
        if self.metric == 'acc':
